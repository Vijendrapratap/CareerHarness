...
from app.domain.scout import ScoutRateLimitError, run_scout, scout_scheduler
...
async def scan_now(
    ...
    """Runs an on-demand Scout scan subject to plan limits (RD-03: 3/day Free, 10/day Pro)."""
    try:
        result = await scout_scheduler.trigger_manual_scan(session, tenant_id)
        await session.commit()  # count the scan and release the write lock before the slow board fetch
        found = await run_scout(session, tenant_id)
        await session.commit()
        return {**result, **found}
